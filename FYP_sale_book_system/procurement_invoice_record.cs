...
namespace FYP_sale_book_system
{
    public partial class procurement_invoice_record : Form
    {
        ...
        private void invoice_info()
        {
            ...
            if (this.resultSYS == 1)
            {
                string SQL = "select invoice_id 'Invoice ID',supplier_id 'Supplier ID',inv.comp_LocationID 'Company LocationID',comp_Name 'Company Name',invoice_no 'Invoice No',supplier_name 'Supplier Name',supplier_address 'Supplier Address',supplier_phone 'Supplier Phone',supplier_dept 'Supplier Department'," +
                               "invoice_date 'Invoice Date',SNID, book_name 'Book Name', book_qty 'Book Qty', receive_status 'Receive Status',remark 'Remark' from invoice_record inv,company c where inv.comp_LocationID=c.comp_LocationID;";
        ...
        public procurement_invoice_record(string uIMode)
        {
            InitializeComponent();
            this.UI_mode = uIMode;
        }

        public procurement_invoice_record()
        {
        }
        ...
        private void procurement_invoice_record_Load(object sender, EventArgs e)
        {
            supplier_info();
            invoice_info();
            supp_inf();
